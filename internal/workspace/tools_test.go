...
func TestRunTool(t *testing.T) {
	t.Run("list root lists files", func(t *testing.T) {
		...
		out, err := workspace.RunTool(d, "list_workspace_directory", `{}`)
	...
	t.Run("read file", func(t *testing.T) {
		...
		out, err := workspace.RunTool(d, "read_workspace_file", `{"path":"a.go"}`)
		require.NoError(t, err)
	...
	t.Run("rejects parent escape", func(t *testing.T) {
		d := t.TempDir()
		_, err := workspace.RunTool(d, "read_workspace_file", `{"path":".."}`)
		require.Error(t, err)
	})

	t.Run("unknown tool", func(t *testing.T) {
		_, err := workspace.RunTool(t.TempDir(), "nope", `{}`)
		require.Error(t, err)
	})

	t.Run("web_search rejects empty query", func(t *testing.T) {
		_, err := workspace.RunTool(t.TempDir(), "web_search", `{"query":""}`)
		require.Error(t, err)
	})
	...
	t.Run("edit_workspace_file", func(t *testing.T) {
		...
		out, err := workspace.RunTool(d, "edit_workspace_file", `{"path":"m.go","old_string":"x := 1","new_string":"x := 2"}`)
		require.NoError(t, err)
	...
	t.Run("grep_workspace", func(t *testing.T) {
		...
		out, err := workspace.RunTool(d, "grep_workspace", `{"pattern":"beta","path":".","max_results":10}`)
		require.NoError(t, err)
	...
	t.Run("write_workspace_file", func(t *testing.T) {
		d := t.TempDir()
		out, err := workspace.RunTool(d, "write_workspace_file", `{"path":"sub/x.txt","content":"hello"}`)
		require.NoError(t, err)
	...
	t.Run("find_workspace_files", func(t *testing.T) {
		...
		out, err := workspace.RunTool(d, "find_workspace_files", `{"suffix":".go","recursive":false}`)
		require.NoError(t, err)
		assert.Contains(t, out, "main.go")
	})

	t.Run("find_workspace_files requires filter", func(t *testing.T) {
		_, err := workspace.RunTool(t.TempDir(), "find_workspace_files", `{}`)
		require.Error(t, err)
	})
}
