...
func TestParseCompletedToolCall_readToolCall(t *testing.T) {
	t.Parallel()
	...
	rec, ok := ParseCompletedToolCall("call-1", raw)
	require.True(t, ok)
...
func TestParseCompletedToolCall_function(t *testing.T) {
	t.Parallel()
	line := `{"function":{"name":"my_tool","arguments":"{}","result":{"ok":true}}}`
	var raw json.RawMessage
	require.NoError(t, json.Unmarshal([]byte(line), &raw))
	rec, ok := ParseCompletedToolCall("fn-1", raw)
	require.True(t, ok)
